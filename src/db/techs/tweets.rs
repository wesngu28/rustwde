...
pub fn move_json_data_to_db(conn: &Connection) {
    let father = fs::read_to_string("src/db/techs/techs.json").unwrap();
    ...
    if let Some(arr) = father.as_array() {
        for value in arr {
            ...
            let path = format!("src/db/techs/json/{}.json", import);
            let wcpath = format!("src/db/techs/wordcounts/{}.json", import);
...
pub fn update_tweet_and_wc(conn: &Connection) {
    let tweets = fs::read_dir("src/db/techs/json").unwrap();
    for (file) in tweets {
        ...
        let file_name = file_path
            ...
            .unwrap()
            .replace(".json", "");
        let wcpath = format!("src/db/techs/wordcounts/{}.json", file_name);
